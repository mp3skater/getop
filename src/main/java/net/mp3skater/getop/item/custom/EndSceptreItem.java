...
public class EndSceptreItem extends TieredItem implements RareItem {
	...
	use(Level level, Player player, InteractionHand hand) {
		...
		// Do the teleportation and boost on server side
			...
			// If the player is on the ground, he boosts and teleports
			if(player.isOnGround()) boostAndTeleport(eye, look, level, player);

			// If he's in the air, he throws the teleport entity
	...
	private void boostAndTeleport(Vec3 eye, Vec3 look, Level level, Player player) {
		...
		if(tp_length>0) {
			GetOP.LOGGER.info("tp length is: {}", tp_length);
		...
		GetOP.LOGGER.info("dash speed is: {}", dash_speed);
		if(dash_speed>0) {
	...
	private void spawnParticles(Level level, Vec3 eye) {
		...
		if (level.isClientSide) {
			...
			// Spawn 20 witch spell particles
			for (int i = 0; i < 20; i++) {
