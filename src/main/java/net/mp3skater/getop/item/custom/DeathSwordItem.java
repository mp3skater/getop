	...
	use(@NotNull Level level, Player player, @NotNull InteractionHand hand) {
		...
		if(level.isClientSide) {
			...
			for (int i = 0; i < 40; i++) {
				...
				double offsetX = margin_xz * Math.cos(angle) * random.nextDouble();
				double offsetY = margin_y * random.nextDouble();
				double offsetZ = margin_xz * Math.sin(angle) * random.nextDouble();
