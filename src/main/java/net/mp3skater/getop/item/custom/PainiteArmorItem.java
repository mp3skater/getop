        ...
        if(!world.isClientSide()) {
            if(hasFullSuitOfArmorOn(player)) {
                if(hasCorrectArmorOn(player)) {
                    DamageSource dmg = player.getLastDamageSource();
                    if (dmg != null) {
                        if (dmg == DamageSource.IN_FIRE || dmg == DamageSource.ON_FIRE || dmg == DamageSource.HOT_FLOOR
                                || dmg == DamageSource.LAVA) {
                        ...
                        }else if (dmg == DamageSource.FLY_INTO_WALL || dmg == DamageSource.FALL) {
                            player.addEffect(new MobEffectInstance(ModEffect.FEATHER_FALL_EFFECT.get(), 200, 1));
                        } else {
